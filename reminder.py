...
def run_morning(update, context):
    ...
    context.job_queue.run_daily(callback_reminder, datetime.time(0, 00, 00), context=(chat_id, update.bot))

def run_night(update, context):
    chat_id = update.message.chat_id
    context.job_queue.run_daily(callback_reminder, datetime.time(11, 33, 00), context=(chat_id, update.bot))
